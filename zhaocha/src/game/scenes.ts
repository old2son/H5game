 ...
 * ===========================================================================*/
import { S, type Diff, type Scene } from './types'
...
const postureDiffs: Diff[] = [
  ...
  {
    name: '孩子坐姿',
    bbox: { x: 184, y: 120, w: 96, h: 150 },
    paint(ctx, v) {
      const bad = v === 1
      const cx = 215
      const headX = bad ? 230 : 215
      const headY = bad ? 182 : 160
      ctx.fillStyle = '#5b8def'
      rr(ctx, cx - 36, bad ? 208 : 198, 72, 86, 22)
      ctx.fill()
      ctx.strokeStyle = '#5b8def'
      ctx.lineWidth = 16
      ctx.lineCap = 'round'
      ctx.beginPath()
      ctx.moveTo(cx - 28, 216)
      ctx.lineTo(cx - 8, 258)
      ctx.moveTo(cx + 28, 216)
      ctx.lineTo(cx + 10, 258)
      ctx.stroke()
      roundDot(ctx, cx - 8, 260, 8, '#ffd9b3')
      roundDot(ctx, cx + 10, 260, 8, '#ffd9b3')
      roundDot(ctx, headX, headY, 25, '#ffd9b3')
      ctx.fillStyle = '#3a2a1d'
      ctx.beginPath()
      ctx.arc(headX, headY, 25, Math.PI * 1.02, Math.PI * 1.98)
      ctx.fill()
      ctx.strokeStyle = '#333'
      ctx.fillStyle = '#333'
      ctx.lineWidth = 2
      if (bad) {
        ctx.beginPath()
        ctx.moveTo(headX - 13, headY - 2)
        ctx.lineTo(headX - 4, headY - 2)
        ctx.moveTo(headX + 4, headY - 2)
        ctx.lineTo(headX + 13, headY - 2)
        ctx.stroke()
      } else {
        roundDot(ctx, headX - 9, headY - 2, 3, '#333')
        roundDot(ctx, headX + 9, headY - 2, 3, '#333')
      }
      ctx.strokeStyle = '#c0392b'
      ctx.lineWidth = 2
      ctx.beginPath()
      ctx.arc(headX, headY + 9, 6, 0.1 * Math.PI, 0.9 * Math.PI)
      ctx.stroke()
    },
  },
  {
    name: '书本距离',
    bbox: { x: 176, y: 224, w: 92, h: 52 },
    paint(ctx, v) {
      const close = v === 1
      const by = close ? 232 : 250
      const tilt = close ? -0.5 : 0
      ctx.save()
      ctx.translate(216, by)
      ctx.rotate(tilt)
      ctx.fillStyle = close ? '#e74c3c' : '#4aa3df'
      rr(ctx, -34, -10, 68, 20, 3)
      ctx.fill()
      ctx.fillStyle = '#fff'
      rr(ctx, -30, -7, 60, 14, 2)
      ctx.fill()
  ...
  {
    name: '墙上视力表',
    bbox: { x: 300, y: 52, w: 76, h: 132 },
    paint(ctx, v) {
      if (v === 1) return // 原图有，含差异图没有
      ctx.fillStyle = '#ffffff'
      rr(ctx, 312, 60, 60, 118, 4)
      ctx.fill()
      ctx.strokeStyle = '#ddd'
      ctx.lineWidth = 1
      rr(ctx, 312, 60, 60, 118, 4)
      ctx.stroke()
      ctx.fillStyle = '#333'
      text(ctx, 'E', 342, 78, 16, '#333')
      text(ctx, 'E', 342, 100, 13, '#333')
      text(ctx, 'E', 342, 120, 11, '#333')
      text(ctx, 'E', 342, 138, 9, '#333')
      text(ctx, 'E', 342, 156, 7, '#333')
    },
  },
  {
    name: '桌面绿植',
    bbox: { x: 156, y: 222, w: 46, h: 46 },
    paint(ctx, v) {
      if (v === 1) return // 原图有绿植，含差异图没有
      ctx.fillStyle = '#c97b4a'
      rr(ctx, 164, 244, 28, 18, 3)
      ctx.fill()
...
const scenePosture: Scene = {
  ...
  diffs: postureDiffs,
  render(ctx, v) {
    drawRoom(ctx, '#fdf3e7', '#f6e2c8', '#d9b38c')
    ctx.fillStyle = '#8a5a32'
    rr(ctx, 196, 250, 70, 14, 4)
    ctx.fill()
    ctx.fillRect(202, 264, 9, 96)
    ctx.fillRect(250, 264, 9, 96)
    drawDesk(ctx, 150, 262, 240)
    postureDiffs[1].paint(ctx, v)
    postureDiffs[2].paint(ctx, v)
    postureDiffs[0].paint(ctx, v)
    postureDiffs[4].paint(ctx, v)
    postureDiffs[3].paint(ctx, v)
  },
}
...
const sceneLight: Scene = {
  ...
  render(ctx, v) {
    ...
    lightDiffs[0].paint(ctx, v)
    lightDiffs[3].paint(ctx, v)
    lightDiffs[2].paint(ctx, v)
    drawDesk(ctx, 150, 264, 230)
    lightDiffs[4].paint(ctx, v)
    lightDiffs[1].paint(ctx, v)
  },
}
...
const sceneDuration: Scene = {
  ...
  render(ctx, v) {
    ...
    durationDiffs[0].paint(ctx, v)
    drawDesk(ctx, 140, 268, 240)
    durationDiffs[2].paint(ctx, v)
    durationDiffs[3].paint(ctx, v)
    durationDiffs[4].paint(ctx, v)
    durationDiffs[1].paint(ctx, v)
  },
}
...
const sceneScreen: Scene = {
  ...
  diffs: screenDiffs,
  render(ctx, v) {
    drawRoom(ctx, v === 1 ? '#2a2f3a' : '#eef1f6', v === 1 ? '#222632' : '#dfe4ee', v === 1 ? '#3a3f4a' : '#cdbfa6')
    if (v === 1) {
      ctx.fillStyle = 'rgba(0,0,0,0.18)'
      ctx.fillRect(0, 0, 400, 400)
    }
    screenDiffs[4].paint(ctx, v)
    screenDiffs[2].paint(ctx, v)
    screenDiffs[1].paint(ctx, v)
    screenDiffs[3].paint(ctx, v)
    screenDiffs[0].paint(ctx, v)
  },
}
...
const outdoorDiffs: Diff[] = [
  ...
  },
  {
    ...
    paint(ctx, v) {
      const few = v === 1
      const trees = few ? [[330, 250, 0.8]] : [[320, 250, 0.9], [360, 240, 1], [388, 260, 0.7]]
      trees.forEach((t) => {
        ctx.fillStyle = '#8a5a32'
...
const sceneOutdoor: Scene = {
  ...
  diffs: outdoorDiffs,
  render(ctx, v) {
    outdoorDiffs[4].paint(ctx, v)
    outdoorDiffs[0].paint(ctx, v)
    ctx.fillStyle = '#8fd06a'
    ctx.fillRect(0, 300, 400, 100)
    ctx.fillStyle = '#79bd57'
    ctx.fillRect(0, 300, 400, 8)
    outdoorDiffs[2].paint(ctx, v)
    outdoorDiffs[1].paint(ctx, v)
    outdoorDiffs[3].paint(ctx, v)
  },
}
...
export const PLACEHOLDER_TIPS: Record<string, string[]> = {
  读写姿势: ['（科普图文占位）眼离书本约 33cm（一尺）。', '胸口离桌沿一拳，握笔离笔尖一寸。', '不趴桌、不歪头，脊柱保持自然直立。'],
  光线环境: ['（科普图文占位）主灯 + 台灯双光源更均匀。', '光线从左上方来，避免手部阴影。', '屏幕避开窗户反光，可拉上遮光帘。'],
  用眼时长: ['（科普图文占位）遵循 20-20-20 法则。', '近距离用眼 20~30 分钟远眺 20 秒。', '连续用眼不超过 1 小时。'],
  屏幕距离: ['（科普图文占位）屏幕距离一臂（50~70cm）。', '屏幕顶端与视线平齐，略向下看。', '开启护眼/滤蓝光模式，亮度随环境。'],
