...
export class CatchGameScene extends Phaser.Scene {
  ...
  private drawWorld() {
    ...
    this.add.text(195, 815, '左右滑动 · 接住好习惯 · 避开坏习惯', {
      fontFamily: 'Microsoft YaHei', fontSize: '11px', color: '#2c6d5a', fontStyle: 'bold',
    }).setOrigin(0.5)
  }

  private createHud() {
    const panel = this.add.rectangle(195, 54, 358, 88, 0x244f83, 0.94).setStrokeStyle(2, 0xffffff, 0.5)
    panel.setDepth(20)
    this.add.text(30, 21, '接睫乐', {
      fontFamily: 'Microsoft YaHei', fontSize: '23px', fontStyle: 'bold', color: '#ffffff',
    }).setDepth(21)
    this.add.text(31, 52, '护眼习惯接接看', {
      fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#cbe9ff',
    }).setDepth(21)
    this.add.text(160, 23, '得分', { fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#bcdcf7' }).setDepth(21)
    this.scoreText = this.add.text(160, 42, '0000', {
      fontFamily: 'Arial', fontSize: '22px', fontStyle: 'bold', color: '#ffe37d',
    }).setDepth(21)
    this.add.text(280, 23, '剩余', { fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#bcdcf7' }).setDepth(21)
    this.timerText = this.add.text(280, 42, '45.0s', {
      fontFamily: 'Arial', fontSize: '20px', fontStyle: 'bold', color: '#ffffff',
    }).setDepth(21)
    this.add.text(30, 88, '生命', { fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#4b6c82' }).setDepth(21)

    for (let i = 0; i < MAX_LIVES; i++) {
      const x = 80 + i * 25
      this.add.text(x, 90, '♡', {
        fontFamily: 'Arial', fontSize: '21px', fontStyle: 'bold', color: '#c8d8e4',
      }).setOrigin(0.5).setDepth(21)
      const heart = this.add.text(x, 90, '♥', {
        fontFamily: 'Arial', fontSize: '21px', fontStyle: 'bold', color: '#ff536b',
      }).setOrigin(0.5).setDepth(22)
      this.hearts.push(heart)
    }
    this.timeBar = this.add.graphics().setDepth(21)
    this.updateTimer()
  }
  ...
  private makeMascotWithBasket(x: number, y: number, size: number) {
    ...
    const basketBody = this.add.rectangle(0, -65 * scale, 104 * scale, 20 * scale, 0xffcf63)
      .setStrokeStyle(3 * scale, 0xb87532)
    const basketRim = this.add.rectangle(0, -78 * scale, 116 * scale, 8 * scale, 0xffe49b)
      .setStrokeStyle(2 * scale, 0xb87532)
    const basketLine1 = this.add.line(-22 * scale, -65 * scale, 0, -9 * scale, 0, 9 * scale, 0xb87532)
      .setLineWidth(Math.max(1, 2 * scale))
    const basketLine2 = this.add.line(22 * scale, -65 * scale, 0, -9 * scale, 0, 9 * scale, 0xb87532)
      .setLineWidth(Math.max(1, 2 * scale))
  ...
  private createIntro() {
    ...
    this.introPanel = this.add.container(195, 444).setDepth(60)
    const shade = this.add.rectangle(0, 0, WIDTH, HEIGHT, 0x183b5b, 0.18)
    const card = this.add.rectangle(0, 0, 344, 594, 0xffffff, 0.985).setStrokeStyle(3, 0x4ca3cb)
    const badge = this.add.circle(0, -232, 59, 0xffedb0).setStrokeStyle(4, 0xffffff)
    const mascot = this.makeMascotWithBasket(0, -228, 96)
    const title = this.add.text(0, -157, '接 睫 乐', {
      fontFamily: 'Microsoft YaHei', fontSize: '35px', fontStyle: 'bold', color: '#315f9a',
    }).setOrigin(0.5)
    const subtitle = this.add.text(0, -116, '45秒护眼习惯挑战', {
      fontFamily: 'Microsoft YaHei', fontSize: '15px', color: '#4b8aab',
    }).setOrigin(0.5)

    const good = this.makeRuleChip(-76, -61, 0x64bf8e, '接住', '好习惯 + 分')
    const bad = this.makeRuleChip(76, -61, 0xf17372, '避开', '坏习惯 - 生命')
    const rules = this.add.text(0, 27, '左右滑动控制角色移动\n初始拥有 5 个生命点\n接错 1 个物品，失去 1 点生命', {
      fontFamily: 'Microsoft YaHei', fontSize: '14px', color: '#465d6b', align: 'center', lineSpacing: 9,
    }).setOrigin(0.5)
    const slogan = this.add.text(0, 103, '近视难可逆，但可防、可控', {
      fontFamily: 'Microsoft YaHei', fontSize: '18px', fontStyle: 'bold', color: '#2e8567',
    }).setOrigin(0.5)
    const best = this.add.text(0, 139, `历史最高分  ${this.best}`, {
      fontFamily: 'Microsoft YaHei', fontSize: '11px', color: '#7e919c',
    }).setOrigin(0.5)
    const start = this.makeButton(0, 190, 226, 56, '开始接取挑战', () => this.startGame())
    const note = this.add.text(0, 247, '结算指数仅依据游戏表现，不是医学筛查或诊断\n科普依据：国家卫生健康委近视防控核心知识', {
      fontFamily: 'Microsoft YaHei', fontSize: '9px', color: '#8b979d', align: 'center', lineSpacing: 4,
    }).setOrigin(0.5)
    this.introPanel.add([shade, card, badge, mascot, title, subtitle, good, bad, rules, slogan, best, start, note])
  }

  private makeRuleChip(x: number, y: number, color: number, heading: string, detail: string) {
    const chip = this.add.container(x, y)
    const bg = this.add.rectangle(0, 0, 136, 55, color, 0.13).setStrokeStyle(2, color, 0.75)
    const headingText = this.add.text(-49, 0, heading, {
      fontFamily: 'Microsoft YaHei', fontSize: '14px', fontStyle: 'bold', color: Phaser.Display.Color.IntegerToColor(color).rgba,
    }).setOrigin(0, 0.5)
    const detailText = this.add.text(2, 0, detail, {
      fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#63737d',
    }).setOrigin(0, 0.5)
    chip.add([bg, headingText, detailText])
    return chip
  }
  ...
  private createItemView(definition: ItemDefinition, x: number, y: number) {
    ...
    const glyph = this.add.text(0, -2, definition.glyph, {
      fontFamily: 'Microsoft YaHei', fontSize: '24px', fontStyle: 'bold', color: '#ffffff',
      stroke: Phaser.Display.Color.IntegerToColor(definition.edge).rgba, strokeThickness: 2,
    }).setOrigin(0.5)
    const labelBg = this.add.rectangle(0, 35, 70, 19, 0xffffff, 0.92).setStrokeStyle(1, definition.edge, 0.55)
    const label = this.add.text(0, 35, definition.label, {
      fontFamily: 'Microsoft YaHei', fontSize: '9px', fontStyle: 'bold', color: '#3e5664',
    }).setOrigin(0.5)
  ...
  private updateItems(delta: number) {
    ...
    for (const item of this.items) {
      ...
      const inBasket = item.view.y >= CATCH_Y - 32 && item.view.y <= CATCH_Y + 17 && Math.abs(item.view.x - this.player.x) < 48
      if (inBasket) {
  ...
  private resolveMiss(item: FallingItem) {
    ...
    if (item.definition.kind === 'bad') {
      ...
      this.score += 10
      this.scoreText.setText(this.score.toString().padStart(4, '0'))
      this.tweens.add({ targets: item.view, alpha: 0, y: '+=18', duration: 170, onComplete: () => item.view.destroy(true) })
    } else {
      this.goodMissed += 1
      this.streak = 0
  ...
  private animateResolved(view: Phaser.GameObjects.Container, color: number, message: string) {
    const feedback = this.add.text(view.x, view.y - 12, message, {
      fontFamily: 'Microsoft YaHei', fontSize: '16px', fontStyle: 'bold', color: '#ffffff',
      stroke: Phaser.Display.Color.IntegerToColor(color).rgba, strokeThickness: 5,
    }).setOrigin(0.5).setDepth(30)
    this.tweens.add({ targets: feedback, y: feedback.y - 48, alpha: 0, duration: 620, ease: 'Cubic.Out', onComplete: () => feedback.destroy() })
  ...
  private showToast(message: string, color: number) {
    ...
    const label = this.add.text(0, 0, message, {
      fontFamily: 'Microsoft YaHei', fontSize: '12px', fontStyle: 'bold', color: '#ffffff',
    }).setOrigin(0.5)
    toast.add([bg, label])
    this.toast = toast
    this.tweens.add({ targets: toast, alpha: 1, y: 140, duration: 150, yoyo: true, hold: 900, onComplete: () => { toast.destroy(true); if (this.toast === toast) this.toast = undefined } })
  }
  ...
  private updateTimer() {
    ...
    this.timeBar.clear().fillStyle(0xdcebf5).fillRoundedRect(236, 91, 126, 7, 3)
    this.timeBar.fillStyle(color).fillRoundedRect(236, 91, 126 * ratio, 7, 3)
  }
  ...
  private showResult(reason: string) {
    ...
    const insight = this.badCaught === 0
      ? '你成功避开了全部坏习惯，动作判断很出色。'
    ...
    const eyebrow = this.add.text(0, -289, '本轮近视风险指数报告', {
      fontFamily: 'Microsoft YaHei', fontSize: '13px', fontStyle: 'bold', color: '#5a7181',
    }).setOrigin(0.5)
    const reasonText = this.add.text(0, -262, reason, {
      fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#87969f',
    }).setOrigin(0.5)
    const ring = this.add.circle(0, -185, 57, color, 0.11).setStrokeStyle(9, color)
    const riskValue = this.add.text(0, -194, String(risk), {
      fontFamily: 'Arial', fontSize: '49px', fontStyle: 'bold', color: Phaser.Display.Color.IntegerToColor(color).rgba,
    }).setOrigin(0.5)
    const riskLabel = this.add.text(0, -155, `${level}风险表现`, {
      fontFamily: 'Microsoft YaHei', fontSize: '12px', fontStyle: 'bold', color: '#4b6170',
    }).setOrigin(0.5)
    const disclaimer = this.add.text(0, -111, '游戏表现指数 ≠ 真实近视风险', {
      fontFamily: 'Microsoft YaHei', fontSize: '11px', fontStyle: 'bold', color: '#c05259',
    }).setOrigin(0.5)

    const statsBg = this.add.rectangle(0, -50, 308, 78, 0xf3f8fb).setStrokeStyle(1, 0xd4e4ed)
    const stats = this.add.text(0, -50, `得分  ${this.score}     接对  ${this.goodCaught}     接错  ${this.badCaught}\n成功避开  ${this.badAvoided}     最高连击  ${this.maxStreak}`, {
      fontFamily: 'Microsoft YaHei', fontSize: '12px', color: '#3f5968', align: 'center', lineSpacing: 10,
    }).setOrigin(0.5)
    const insightTitle = this.add.text(-142, 12, '表现解读', {
      fontFamily: 'Microsoft YaHei', fontSize: '12px', fontStyle: 'bold', color: '#315f9a',
    }).setOrigin(0, 0.5)
    const insightText = this.add.text(-142, 37, insight, {
      fontFamily: 'Microsoft YaHei', fontSize: '11px', color: '#5c6e78', wordWrap: { width: 284, useAdvancedWrap: true }, lineSpacing: 4,
    }).setOrigin(0, 0)
    const tipBg = this.add.rectangle(0, 112, 308, 88, 0xeaf7f0).setStrokeStyle(1, 0xb9dfca)
    const tipTitle = this.add.text(-137, 83, '带走一个护眼行动', {
      fontFamily: 'Microsoft YaHei', fontSize: '11px', fontStyle: 'bold', color: '#2b855e',
    }).setOrigin(0, 0.5)
    const tip = this.add.text(-137, 104, TIPS[tipIndex], {
      fontFamily: 'Microsoft YaHei', fontSize: '11px', color: '#416555', wordWrap: { width: 274, useAdvancedWrap: true }, lineSpacing: 4,
    }).setOrigin(0, 0)
    const slogan = this.add.text(0, 174, '近视难可逆，但可防、可控', {
      fontFamily: 'Microsoft YaHei', fontSize: '17px', fontStyle: 'bold', color: '#2e8567',
    }).setOrigin(0.5)
    const restart = this.makeButton(0, 226, 218, 52, '再挑战一次', () => this.startGame())
    const bestText = this.add.text(0, 272, `历史最高分 ${this.best}`, {
      fontFamily: 'Microsoft YaHei', fontSize: '10px', color: '#81919a',
    }).setOrigin(0.5)
    const note = this.add.text(0, 302, '如发现看远模糊、眯眼等情况，请及时进行专业检查', {
      fontFamily: 'Microsoft YaHei', fontSize: '9px', color: '#89969c',
    }).setOrigin(0.5)
    panel.add([shade, card, eyebrow, reasonText, ring, riskValue, riskLabel, disclaimer, statsBg, stats, insightTitle, insightText, tipBg, tipTitle, tip, slogan, restart, bestText, note])
  ...
  private makeButton(x: number, y: number, width: number, height: number, text: string, onClick: () => void) {
    ...
    const shadow = this.add.rectangle(0, 5, width, height, 0x24476c, 0.2)
    const bg = this.add.rectangle(0, 0, width, height, 0x347eb2).setStrokeStyle(2, 0x9bd3ef)
    const label = this.add.text(0, 0, text, {
      fontFamily: 'Microsoft YaHei', fontSize: '15px', fontStyle: 'bold', color: '#ffffff',
    }).setOrigin(0.5)
